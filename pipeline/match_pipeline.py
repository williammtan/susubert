...
def match_pipeline(
    ...
    if cache_matches_table is not None:
        ...
        fin_task = fin(matches=merge_cache_task.output, products=preprocess_task.outputs['products'], min_cluster_size=min_cluster_size).set_gpu_limit(1)
    else:
        fin_task = fin(matches=matcher_task.output, products=preprocess_task.outputs['products'], min_cluster_size=min_cluster_size).set_gpu_limit(1)

    save_clusters_task = save_clusters(clusters=fin_task.output, products=preprocess_task.outputs['products'])
